<template>
  <div>
    ...
    <div v-else-if="!items.length" class="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center text-gray-500">No customers found.</div>
    ...
    <div v-if="formVisible" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="formVisible = false">
      <div class="bg-white rounded-xl shadow-2xl p-6 max-w-lg w-full mx-4">
        <h3 class="text-lg font-semibold text-gray-800 mb-4">{{ editingItem ? 'Edit' : 'Add' }} Customer</h3>
...
function openForm(item) {
  ...
  form.name = item?.name || ''; form.email = item?.email || ''; form.phone = item?.phone || ''; form.company = item?.company || ''; form.address = item?.address || ''
...
async function handleDelete() {
  deleting.value = true
  try { await axios.delete(`/customers/${deleteItem.value.id}`); items.value = items.value.filter(i => i.id !== deleteItem.value.id); deleteItem.value = null }
