<template>
  <div>
    ...
    <div v-else-if="!items.length" class="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center text-gray-500">No purchase orders found.</div>
    <div v-else class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      <table class="w-full text-sm">
        <thead class="bg-gray-50 text-gray-600">
          <tr><th class="text-left px-4 py-3 font-medium">PO #</th><th class="text-left px-4 py-3 font-medium">Supplier</th><th class="text-left px-4 py-3 font-medium">Date</th><th class="text-left px-4 py-3 font-medium">Total</th><th class="text-left px-4 py-3 font-medium">Status</th><th class="text-right px-4 py-3 font-medium">Actions</th></tr>
        </thead>
    ...
    <div v-if="formVisible" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="formVisible = false">
      <div class="bg-white rounded-xl shadow-2xl p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <h3 class="text-lg font-semibold text-gray-800 mb-4">{{ editingItem ? 'Edit' : 'New' }} Purchase Order</h3>
        ...
        <form @submit.prevent="handleSave" class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <select v-model="form.supplier_id" required class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm">
    ...
    <div v-if="deleteItem" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="deleteItem = null">
      <div class="bg-white rounded-xl shadow-2xl p-6 max-w-sm w-full mx-4">
        ...
        <p class="text-gray-600 text-sm mb-4">Delete PO #{{ deleteItem.id }}? This cannot be undone.</p>
...
async function fetchData() {
  loading.value = true
  try { const params = {}; if (filters.status) params.status = filters.status; const { data } = await axios.get('/purchase-orders', { params }); items.value = data.data || data }
...
function addItem() { form.items.push({ raw_material_id: '', quantity: 1, unit_price: 0 }) }
...
async function handleDelete() {
  deleting.value = true
  try { await axios.delete(`/purchase-orders/${deleteItem.value.id}`); items.value = items.value.filter(i => i.id !== deleteItem.value.id); deleteItem.value = null }
