<template>
  <div>
    ...
    <div v-else-if="!items.length" class="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center text-gray-500">No targets found.</div>
    ...
    <div v-if="formVisible" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="formVisible = false">
      <div class="bg-white rounded-xl shadow-2xl p-6 max-w-lg w-full mx-4">
        <h3 class="text-lg font-semibold text-gray-800 mb-4">{{ editingItem ? 'Edit' : 'Add' }} Target</h3>
        <div v-if="formError" class="mb-4 bg-red-50 text-red-700 px-4 py-3 rounded-lg text-sm">{{ formError }}</div>
        <form @submit.prevent="handleSave" class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Product</label>
            <select v-model="form.product_id" required class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm">
          ...
          <div class="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" :disabled="saving" class="px-6 py-2.5 text-sm text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 disabled:opacity-50">{{ saving ? 'Saving...' : 'Save' }}</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <div v-if="deleteItem" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="deleteItem = null">
      <div class="bg-white rounded-xl shadow-2xl p-6 max-w-sm w-full mx-4">
        ...
        <p class="text-gray-600 text-sm mb-4">Delete this target? This cannot be undone.</p>
...
async function fetchData() {
  loading.value = true
  try {
    const [tRes, pRes] = await Promise.all([axios.get('/production-targets'), axios.get('/products')])
    items.value = tRes.data.data || tRes.data
    products.value = pRes.data.data || pRes.data
  } catch (e) { error.value = 'Failed to load targets.' }
...
async function handleSave() {
  ...
  try {
    if (editingItem.value) {
      const { data } = await axios.put(`/production-targets/${editingItem.value.id}`, form)
...
async function handleDelete() {
  deleting.value = true
  try {
    await axios.delete(`/production-targets/${deleteItem.value.id}`)
