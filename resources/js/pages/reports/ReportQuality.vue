        ...
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-for="check in report.checks" :key="check.id" class="hover:bg-gray-50">
              ...
              <td class="px-4 py-3">{{ formatDate(check.check_date) }}</td>
              <td class="px-4 py-3">
                <span :class="check.status === 'passed' ? 'bg-green-100 text-green-700' : check.status === 'failed' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'" class="px-2 py-1 rounded-full text-xs font-medium">{{ humanize(check.status) }}</span>
              </td>
            </tr>
          </tbody>
        </table>
