<template>
  <div>
    ...
    <div v-else-if="!items.length" class="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center text-gray-500">No warehouses found.</div>
    ...
    <div v-if="formVisible" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="formVisible = false">
      <div class="bg-white rounded-xl shadow-2xl p-6 max-w-lg w-full mx-4">
        <h3 class="text-lg font-semibold text-gray-800 mb-4">{{ editingItem ? 'Edit' : 'Add' }} Warehouse</h3>
        ...
        <form @submit.prevent="handleSave" class="space-y-4">
          ...
          <div>
            ...
            <input v-model="form.location" class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Capacity</label>
            <input v-model.number="form.capacity" type="number" min="0" class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm" />
          </div>
...
async function fetchData() {
  ...
  } catch (e) { error.value = 'Failed to load warehouses.' }
...
async function handleSave() {
  ...
  try {
    if (editingItem.value) {
      const { data } = await axios.put(`/warehouses/${editingItem.value.id}`, form)
...
async function handleDelete() {
  deleting.value = true
  try {
    await axios.delete(`/warehouses/${deleteItem.value.id}`)
